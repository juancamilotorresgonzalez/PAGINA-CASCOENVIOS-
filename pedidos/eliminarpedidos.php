<?php
 include('../conexion/conexion.php');
 include("../modelos/pedidosmodelo.php");
?>
<?php
$obj = new pedidos();
if($_POST){
    $obj->codigoPedido = $_POST['codigoPedido'];
    $obj->fechaPedido = $_POST['fechaPedido'];
    $obj->horaPedido = $_POST['horaPedido'];
    $obj->documentoCliente = $_POST['documentoCliente'];
}
?>
<?php
$llave = $_GET['key'];
echo $llave;
$c = new Conexion();
$cone = $c->conectar_al_servidor();
$sql = "select * from pedidos where codigoPedido = '$llave'";
$resultado = mysqli_query($cone,$sql);
if($arreglo = mysqli_fetch_row($resultado)){
    $obj->codigoPedido = $arreglo[0];
    $obj->fechaPedido = $arreglo[1];
    $obj->horaPedido = $arreglo[2];
    $obj->documentoCliente = $arreglo[3];
}
else{
    $obj->codigoPedido = null;
    $obj->fechaPedido = null;
    $obj->horaPedido = null;
    $obj->documentoCliente = null;
}
?>


<!Doctype html> 
<html lang="es">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="idth=device-width, initial-scale=1, shrink-to-fit=no"> 
        <link rel="stylesheet" href="css/bootstrap.min.css">
        <link rel="stylesheet" href="css/font-awesome.min.css">
        <link rel="stylesheet" href="css/julios.css">
         	
        <title>Modulo Pedidos</title>
    </head>
    <br>
    <br>
    <body>
        <center>
            <form action="" name="pedidos" method="POST">
                <table border="1">
                <tr>
                            <td colspan="2"><center>pedidos</center></td>
                        </tr>
                        <tr>
                            <td>Código</td>
                            <td><input size="10" type="number" name="codigoPedido" id="codigoPedido" placeholder="Código es Automatico"></td>
                        </tr>
                        <tr>
                            <td>Fecha</td>
                            <td><input size="40" type="date" name="fechaPedido" id="fechaPedido" placeholder="Fecha del pedido"></td>
                        </tr>
                        <tr>
                            <td>Hora</td>
                            <td><input size="40" type="time" name="horaPedido" id="horaPedido" placeholder="Hora del pedido"></td>
                        </tr>
                        <tr>
                            <td>documentoCliente</td>
                            <td><input size="40" type="text" name="documentoCliente" id="documentoCliente" placeholder="Documento del cliente"></td>
                        </tr>
                        <tr>
                            <td>Categoria</td>
                            <td>
                            <?php   
                                            $c = new Conexion();
                                            $cone = $c->conectar_al_servidor();
                                            $p2 = "select nombreCategorias from categorias where idcategorias='$obj->idcategorias'";
                                            $res2 = mysqli_query($cone,$p2);
                                            $arreglo2 = mysqli_fetch_row($res2);
                                            echo $arreglo2[0];
                                    ?>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2">
                                <center>
                                    <button type="submit" name="elimina"> Eliminar</button>
                                    <a href="pedidos.php">
                                             <button type="button" name="salir">Salir</button>
                                    </a>
                                </center>
                            </td>
                        </tr>
                </table>
            </form>
        </center>  
    </body> 
</html>